...
public class Worker implements Runnable {
    ...
    public void run() {
        ...
        if (config.freeze()) {
            ...
            for (Player player : playerData.getPlayersByRole(ManHuntRole.HUNTED)) {
    ...
    private void updateCompass(Player player) {
        Player nearest = getNearestHunted(player);
        if (nearest == null) {
    ...
    private Player getNearestHunted(Player player) {

        Location playerLocation = player.getLocation();

        return plugin.getServer().getOnlinePlayers().stream()
                .filter(p -> !p.equals(player))
                .filter(p -> playerData.getRole(p) == ManHuntRole.HUNTED)
    ...
    public void drawDirection(Location point1, Location point2, double space) {
        // Draws a single particle in direction of hunted (only X & Z coordinates)
        World world = point1.getWorld();
        Validate.isTrue(Objects.equals(point2.getWorld(), world), "You have to be in same worlds!");
        Vector p1 = point1.toVector();
